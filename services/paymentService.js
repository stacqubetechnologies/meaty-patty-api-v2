...
exports.createPaymentIntent = async ({ amount, currency }) => {
    try {
        ...
        let amountInCents = amount * 100; // Default conversion to cents
        if (currency.toLowerCase() === 'eur') {
            amountInCents = amount * 100; // 2 EUR = 200 cents
        } else if (currency.toLowerCase() === 'usd') {
            amountInCents = amount * 100; // 2 USD = 200 cents
        } else if (currency.toLowerCase() === 'gbp') {
            amountInCents = amount * 100; // 2 GBP = 200 pence
        } else {
        ...
        const totalAmount = amountInCents + serviceCharge;
